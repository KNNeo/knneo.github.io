	...
	for(let box of document.querySelectorAll('.item_box .item'))
	{
		...
		if(item.getBoundingClientRect().width == item.getBoundingClientRect().height)
			...
			list.push({
                ...
                orientation: itemRect.height / itemRect.width <= 1 ? '横向き' : '縦向き'
			});
		}
	}
	console.log(list);
...
action.addEventListener('click', condenseList);
action.href = 'javascript:void(0);';
action.innerText = 'CONDENSE';
document.querySelector('.is-search').appendChild(action);
