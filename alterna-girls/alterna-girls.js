...
function getAge(DOB) {
    ...
    return Math.floor((new Date().getTime() - birthDate) / 31536000000);
}
...
{
 //wantedListString += "<li><a href=\"#" + profileName.replace(" ","") + "\">" + profileName + "</a></li>";
...
function createCalendar(monthNo, DOBlist) {
    ...
    for (var item of DOBlist) {
        ...
        if (htmlString.indexOf(month[birthdayInYear.getMonth()]) > -1 && htmlString.indexOf("<td>" + birthdayInYear.getDate() + "</td>") > -1 && item.name != "Me")
            htmlString = htmlString.replace("<td>" + birthdayInYear.getDate() + "</td>", "<td style=\"color: #00e4ff;\"><div class=\"popitem\" style=\"display: none; margin: 4px;\">" + item.name + " turns " + item.currentAge + "! (" + birthdayInYear.getDate() + " " + month[birthdayInYear.getMonth()].substring(0, 3) + ")</div>" + birthdayInYear.getDate() + "</td>");
        else if (htmlString.indexOf(month[birthdayInYear.getMonth()]) > -1)
            //console.error("Did not write " + item.date + " into " + month[birthdayInYear.getMonth()] + " calendar");
            //replace "</div>(date)</td>" with "<br />(new content)</div>(date)</td>"
            htmlString = htmlString.replace("</div>" + birthdayInYear.getDate() + "</td>", "<br />" + item.name + " turns " + item.currentAge + "! (" + birthdayInYear.getDate() + " " + month[birthdayInYear.getMonth()].substring(0, 3) + ")</div>" + birthdayInYear.getDate() + "</td>");
    }
...
var statusPopup = "<div class=\"tp-description\">As answered haphazardly by Uesaka Sumire (and expanded on by me) the three \"turning points\" of a voice actress :<br/>~ Singer Debut (The exhibition of their unique voices in singing)<br/>~ Swimsuit Photobook (The display of their figure to the extent of being half-naked)<br/>~ Married (The declaration of the end of idolism)</div>";
for (var statusPopOut of document.getElementsByClassName("turning-point"))
statusPopOut.addEventListener("mouseover", function(d) {
    if (event.target.parentElement.getElementsByClassName("tp-description").length > 0) event.target.parentElement.getElementsByClassName("tp-description")[0].remove();
    event.target.innerHTML = statusPopup + event.target.innerHTML;
});