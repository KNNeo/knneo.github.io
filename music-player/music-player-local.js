...
function onChangeOption() {
	...
	if(id > 0)
	{
		...
		queryDb("SELECT KNID as ID, KNYEAR, Filename, SongTitle as 'Song Title', ArtistTitle as 'Artist Title', ReleaseTitle as 'Release Title', ReleaseArtistTitle as 'Release Artist', ReleaseYear as 'Year', Rating, Genre, DateCreated as 'Date Added', VocalCode as 'Vocal Code', LanguageCode as 'Language', LyricsURL as 'Lyrics', SongTitleAlt as 'Song Title (" + altTitleTag + ")', ArtistID, ReleaseID FROM Song WHERE KNID = " + id, generateLayout);
	}
...
function queryInfo(contents) {
	...
	query = "SELECT KNYEAR, Category, Type, ReleaseTitle as 'Release Title', ReleaseArtistTitle as 'Release Artist', TracksSelected || '/' || TracksTotal AS 'Tracks In Library', ReleaseYear || SUBSTR('0000' || ReleaseDate, -4, 4) AS 'Release Date', ReleaseTitleAlt as 'Release Title (" + altTitleTag + ")', ReleaseArtistTitleAlt as 'Release Artist (" + altTitleTag + ")' FROM Release WHERE ReleaseID = " + row[columnIndexReleaseID];
