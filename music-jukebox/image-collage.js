...
//--VARIABLES--//
let minWidth = 180;			// minimum width for thumbnail display
...
function generateGrid() {
	...
	.filter(m => window['searchCriteria'].length == 0 || searchArray.filter(s => m.includes(s)).length == searchArray.length)
	.sort(function(a,b) {
...
function calculateThumbnailSize() {
	...
	while(screenWidth > window['thumbWidth']) {	screenWidth -= window['thumbWidth']; }
