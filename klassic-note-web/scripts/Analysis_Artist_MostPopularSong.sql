...
(select 20*count(*) from Ranking r where r.SongID = s.ID and r.RankNo between 2 and 9) +
...
from Song s
where s.ArtistTitle = 'Asakura Momo' and Total > 0 order by Total desc, KNYEAR desc
)